...
#EEG .set fájl betöltése
raw = mne.io.read_raw_eeglab('src/sub-001_eeg.set', preload=True)

#Szűrés (1–40 Hz sáv)
raw.filter(1., 40., fir_design='firwin')

#Események kinyerése annotációkból, ha vannak
if raw.annotations:
...
#Csak az első 90 másodperc megtartása (gyors teszteléshez)
raw.crop(tmax=90)
events = events[events[:, 0] <= raw.last_samp]

#Epoch-olás (2 másodperces ablakok)
epochs = mne.Epochs(raw, events, event_id=event_id, tmin=0, tmax=2.0, baseline=None, preload=True)

#Jellemzők kinyerése frekvenciasávok alapján
